...
void Model::createModelFSM()
{
    ...
    GameStep * FSMTempMatrix[STATE_COUNT][EVENT_COUNT] = //Creamos matriz temporal para luego copiar a la final
    {
        ...
        {idle,                           idle,                         idle,                         waitingSecondAction, idle,                          idle,                          idle,                             idle,                           idle,                           idle,                                waitingResponse,     waitingResponse,      idle,                        idle,                          waitingFirstAction,      idle,                         idle,                                 idle,                                playAgain,        playAgain,                waitingFirstAction,   waitingFirstAction, idle,                        idle                       }, //WAITING FOR FIRST ACTION
...
void Model::fillGraphicsData(View* view, gameData_t * gameData)
{
    ...
    for (unsigned floor = 0; floor < V_TOTAL_FLOORS; floor++)
    {
        if (gamePointers->floors[floor]->isGuardActive())
        {
            ...
            view->graphicsData->guards[floor].guardDie = (locationV_t) (gamePointers->floors[floor]->getGuardDestination() + floor * FLOORTILE_QTY);
